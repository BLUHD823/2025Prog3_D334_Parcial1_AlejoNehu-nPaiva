...
function imprimirDatosAlumno() { //Imprime los datos del alumno en un texto o label específico
    label.textContent = `DNI: ${alumno.dni} - NOMBRE COMPLETO: ${alumno.nombre} ${alumno.apeliido}`;
    console.log(`DNI: ${alumno.dni} - NOMBRE COMPLETO: ${alumno.nombre} ${alumno.apeliido}`);
}

//EJERCICIO 3: MOSTRAR FRUTAS
function mostrarLista(array) //Muestra en disintas cartas el array de productos 
{
    let htmlProductos = "";
    array.forEach(fruta=>{ 
        htmlProductos += `
    ...
    listaFrutas.innerHTML = htmlProductos;
}
...
function agregarCarrito(idFruta) { //Encargada de agregar el producto al carrito
    ...
    carrito.push(frutas.find(fruta => fruta.id == idFruta));
    mostrarCarrito();
    actualizarCarrito()
}
...
function cantidadProductos() { //Modifica un texto para que muestre la cantidad de productos en el carrito
...
function totalCarrito() { //Suma todos los precios de los productos del carrito
    ...
    totalTexto.textContent = `TOTAL: $ ${numeroTotal}`;
}

//EJERCICIO 8: Botones ordenar
function ordenarNombre() { //Ordena la lista de productos por nombre
    frutas.sort(function(a, b){return a.nombre.localeCompare(b.nombre)});
    mostrarLista(frutas);
}

function ordenarMenorAMayor() { //Ordena la lista de mayor a menor 
    frutas.sort(function(a, b){return a.precio - b.precio});
    mostrarLista(frutas);
}
